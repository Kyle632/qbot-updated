...
if(config.api) {
    ...
    app.get('/user', async (req, res) => {
        ...
        try {
            const username = await robloxClient.getUsernameFromId(Number(id));
            const robloxUser = { userId: Number(id), username: username };

            const userData = await provider.findUser(robloxUser.userId.toString());
    ...
    app.post('/promote', async (req, res) => {
        ...
        try {
            const username = await robloxClient.getUsernameFromId(Number(id));
            const robloxMember = { userId: Number(id), username: username };
            const currentRank = await robloxClient.getRankInGroup(config.groupId, Number(id));
            if(currentRank === 0) throw new Error(); // 0 means not in group
            const groupRoles = await robloxClient.getRoles(config.groupId);
            const currentRoleIndex = groupRoles.findIndex((role) => role.rank === currentRank);
            const role = groupRoles[currentRoleIndex + 1];
            if(!role) throw new Error();
            await robloxClient.setRank(config.groupId, Number(id), role.id);
            const currentRoleName = groupRoles[currentRoleIndex].name;
            logAction('Promote', 'API Action', robloxMember.username, robloxMember, `${currentRoleName} (${currentRank}) → ${role.name} (${role.rank})`);
            return res.send({ success: true });
        } catch (err) {
    ...
    app.post('/demote', async (req, res) => {
        ...
        try {
            ...
            await robloxClient.setRank(config.groupId, Number(id), role.id);
            const currentRoleName = groupRoles[currentRoleIndex].name;
            logAction('Demote', 'API Action', robloxMember.username, robloxMember, `${currentRoleName} (${currentRank}) → ${role.name} (${role.rank})`);
            return res.send({ success: true });
        } catch (err) {
            return res.send({ success: false, msg: 'Failed to rank.' });
        }
    ...
    app.post('/fire', async (req, res) => {
        ...
        try {
            ...
            if(!role) throw new Error();
            await robloxClient.setRank(config.groupId, Number(id), role.id);
            logAction('Fire', 'API Action', null, robloxMember, `${currentRoleName} (${currentRank}) → ${role.name} (${role.rank})`);
            return res.send({ success: true });
        } catch (err) {
            return res.send({ success: false, msg: 'Failed to rank.' });
        }
    ...
    app.post('/setrank', async (req, res) => {
        ...
        try {
            ...
            if(currentRank === 0) throw new Error(); // 0 means not in group
            const groupRoles = await robloxClient.getRoles(config.groupId);
            const currentRoleIndex = groupRoles.findIndex((r) => r.rank === currentRank);
            const currentRoleName = groupRoles[currentRoleIndex].name;
            ...
            await robloxClient.setRank(config.groupId, Number(id), newRole.id);
            logAction('Set Rank', 'API Action', null, robloxMember, `${currentRoleName} (${currentRank}) → ${newRole.name} (${newRole.rank})`);
            return res.send({ success: true });
        } catch (err) {
            return res.send({ success: false, msg: 'Failed to rank.' });
        }
    ...
    app.post('/suspend', async (req, res) => {
        ...
        try {
            ...
            const groupRoles = await robloxClient.getRoles(config.groupId);
            const currentRoleIndex = groupRoles.findIndex((r) => r.rank === currentRank);
            const currentRole = groupRoles[currentRoleIndex];
            const role = groupRoles.find((role) => role.rank === config.suspendedRank);
            if(!role) throw new Error();

            const userData = await provider.findUser(robloxMember.userId.toString());
            ...
            logAction('Suspend', 'API Action', null, robloxMember, `${currentRole.name} (${currentRank}) → ${role.name} (${role.rank})`, endDate);
            await provider.updateUser(robloxMember.userId.toString(), { suspendedUntil: endDate, unsuspendRank: currentRole.id });
    ...
    app.post('/unsuspend', async (req, res) => {
        ...
        try {
            ...
            if(currentRole.id !== userData.unsuspendRank) {
                await robloxClient.setRank(config.groupId, Number(id), userData.unsuspendRank);
            }

            logAction('Unsuspend', 'API Action', null, robloxMember, `${currentRole.name} (${currentRank}) → ${role.name} (${role.rank})`);
            await provider.updateUser(robloxMember.userId.toString(), { suspendedUntil: null, unsuspendRank: null });
    ...
    app.post('/xp/add', async (req, res) => {
        ...
        try {
            ...
            await provider.updateUser(robloxMember.userId.toString(), { xp });
    ...
    app.post('/xp/rankup', async (req, res) => {
        ...
        try {
            ...
            const currentRoleIndex = groupRoles.findIndex((r) => r.rank === currentRank);
            const currentRole = groupRoles[currentRoleIndex];
            // Create a member object with role property for findEligibleRole compatibility
            const memberWithRole = { ...robloxMember, role: { rank: currentRank } };
            const userData = await provider.findUser(robloxMember.userId.toString());
            const role = await findEligibleRole(memberWithRole, groupRoles, userData.xp);
            if(!role) return res.send({ success: false, msg: 'No rankup available.' });

            await robloxClient.setRank(config.groupId, robloxMember.userId, role.id);
            logAction('XP Rankup', 'API Action', null, robloxMember, `${currentRole.name} (${currentRank}) → ${role.name} (${role.rank})`);
    ...
    app.post('/shout', async (req, res) => {
        ...
        if(!content) content = '';
        try {
            await robloxClient.shout(config.groupId, content);
    ...
    app.post('/join-requests/accept', async (req, res) => {
        ...
        try {
            ...
            await robloxClient.handleJoinRequest(config.groupId, robloxUser.userId, true);
    ...
    app.post('/join-requests/deny', async (req, res) => {
        ...
        try {
            ...
            await robloxClient.handleJoinRequest(config.groupId, robloxUser.userId, false);
