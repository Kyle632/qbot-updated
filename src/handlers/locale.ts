...
const getHeadshotImage = async (userId: number) => {
    const thumbnails = await robloxClient.getPlayerThumbnail(userId, '48x48', 'png', false, 'headshot');
    return thumbnails[0];
}
...
export const getSuccessfulAddingAndRankupEmbed = async(user: any, newRole: string, xpChange: string): Promise<EmbedBuilder> => {
    const embed = new EmbedBuilder()
        .setAuthor({ name: 'Success!', iconURL: checkIconUrl })
        .setColor(greenColor)
        .setThumbnail((await getHeadshotImage(user.userId)).imageUrl)
        .setDescription(`**${user.username}** has been given **${xpChange}** XP and has been promoted to **${newRole}**, becuase they had enough XP!`)

    return embed
}

export const getSuccessfulPromotionEmbed = async (user: any, newRole: string): Promise<EmbedBuilder> => {
    const embed = new EmbedBuilder()
        .setAuthor({ name: 'Success!', iconURL: checkIconUrl })
        .setColor(greenColor)
        .setThumbnail((await getHeadshotImage(user.userId)).imageUrl)
        .setDescription(`**${user.username}** has been successfully promoted to **${newRole}**!`);

    return embed;
}

export const getSuccessfulDemotionEmbed = async (user: any, newRole: string): Promise<EmbedBuilder> => {
    const embed = new EmbedBuilder()
    .setAuthor({ name: 'Success!', iconURL: checkIconUrl })
        .setColor(greenColor)
        .setThumbnail((await getHeadshotImage(user.userId)).imageUrl)
        .setDescription(`**${user.username}** has been successfully demoted to **${newRole}**.`);

    return embed;
}

export const getSuccessfulFireEmbed = async (user: any, newRole: string): Promise<EmbedBuilder> => {
    const embed = new EmbedBuilder()
        .setAuthor({ name: 'Success!', iconURL: checkIconUrl })
        .setColor(greenColor)
        .setThumbnail((await getHeadshotImage(user.userId)).imageUrl)
        .setDescription(`**${user.username}** has been successfully fired, and now has the **${newRole}** role.`);

    return embed;
}

export const getSuccessfulExileEmbed = async (user: any): Promise<EmbedBuilder> => {
    const embed = new EmbedBuilder()
        .setAuthor({ name: 'Success!', iconURL: checkIconUrl })
        .setColor(greenColor)
        .setThumbnail((await getHeadshotImage(user.userId)).imageUrl)
        .setDescription(`**${user.username}** has been successfully exiled from the group.`);

    return embed;
}

export const getSuccessfulSetRankEmbed = async (user: any, newRole: string): Promise<EmbedBuilder> => {
    const embed = new EmbedBuilder()
        .setAuthor({ name: 'Success!', iconURL: checkIconUrl })
        .setColor(greenColor)
        .setThumbnail((await getHeadshotImage(user.userId)).imageUrl)
        .setDescription(`**${user.username}** has successfully been ranked to the **${newRole}** role.`);
...
export const getSuccessfulXPRankupEmbed = async (user: any, newRole: string): Promise<EmbedBuilder> => {
    const embed = new EmbedBuilder()
        .setAuthor({ name: 'Success!', iconURL: checkIconUrl })
        .setColor(greenColor)
        .setThumbnail((await getHeadshotImage(user.userId)).imageUrl)
        .setDescription(`**${user.username}** has been successfully ranked to **${newRole}**!`);

    return embed;
}

export const getSuccessfulXPChangeEmbed = async (user: any, xp: number): Promise<EmbedBuilder> => {
    const embed = new EmbedBuilder()
        .setAuthor({ name: 'Success!', iconURL: checkIconUrl })
        .setColor(greenColor)
        .setThumbnail((await getHeadshotImage(user.userId)).imageUrl)
        .setDescription(`The XP of **${user.username}** has been updated, they now have a total of **${xp}** XP.`);

    return embed;
}

export const getSuccessfulSuspendEmbed = async (user: any, newRole: string, endDate: Date): Promise<EmbedBuilder> => {
    const embed = new EmbedBuilder()
        .setAuthor({ name: 'Success!', iconURL: checkIconUrl })
        .setColor(greenColor)
        .setThumbnail((await getHeadshotImage(user.userId)).imageUrl)
        .setDescription(`**${user.username}** has been successfully suspended, and will have their rank returned in <t:${Math.round(endDate.getTime() / 1000)}:R>.`);

    return embed;
}

export const getSuccessfulUnsuspendEmbed = async (user: any, newRole: string): Promise<EmbedBuilder> => {
    const embed = new EmbedBuilder()
        .setAuthor({ name: 'Success!', iconURL: checkIconUrl })
        .setColor(greenColor)
        .setThumbnail((await getHeadshotImage(user.userId)).imageUrl)
        .setDescription(`**${user.username}** is no longer suspended, and has been ranked back to **${newRole}**!`);

    return embed;
}

export const getSuccessfulAcceptJoinRequestEmbed = async (user: any): Promise<EmbedBuilder> => {
    const embed = new EmbedBuilder()
        .setAuthor({ name: 'Success!', iconURL: checkIconUrl })
        .setColor(greenColor)
        .setThumbnail((await getHeadshotImage(user.userId)).imageUrl)
        .setDescription(`The join request from **${user.username}** has been accepted.`);

    return embed;
}

export const getSuccessfulDenyJoinRequestEmbed = async (user: any): Promise<EmbedBuilder> => {
    const embed = new EmbedBuilder()
        .setAuthor({ name: 'Success!', iconURL: checkIconUrl })
        .setColor(greenColor)
        .setThumbnail((await getHeadshotImage(user.userId)).imageUrl)
        .setDescription(`The join request from **${user.username}** has been denied.`);
...
export const getShoutLogEmbed = async (shout: any): Promise<EmbedBuilder> => {
    const shoutCreator = await robloxClient.getUsernameFromId(shout.creator.id);
    const embed = new EmbedBuilder()
        .setAuthor({ name: `Shout from ${shoutCreator}`, iconURL: quoteIconUrl })
...
export const getWallPostEmbed = async (post): Promise<EmbedBuilder> => {
    const postCreator = await robloxClient.getUsernameFromId(post.poster);
    const embed = new EmbedBuilder()
        .setAuthor({ name: `Posted by ${postCreator}`, iconURL: quoteIconUrl })
...
export const getLogEmbed = async (action: string, moderator: DiscordUser | any, reason?: string, target?: any, rankChange?: string, endDate?: Date, body?: string, xpChange?: string): Promise<EmbedBuilder> => {
...
export const getPartialUserInfoEmbed = async (user: any, data: DatabaseUser): Promise<EmbedBuilder> => {
    const primaryGroup = null; // TODO: Implement getPrimaryGroup equivalent in Noblox.js
    const embed = new EmbedBuilder()
        .setAuthor({ name: `Information: ${user.username}`, iconURL: infoIconUrl })
        .setColor(mainColor)
...
export const getUserInfoEmbed = async (user: any, member: any, data: DatabaseUser): Promise<EmbedBuilder> => {
    const primaryGroup = null; // TODO: Implement getPrimaryGroup equivalent in Noblox.js
    const embed = new EmbedBuilder()
        .setAuthor({ name: `Information: ${user.username}`, iconURL: infoIconUrl })
        .setColor(mainColor)
        .setDescription(primaryGroup ? `Primary Group: [${primaryGroup.group.name}](https://roblox.com/groups/${primaryGroup.group.id})` : null)
        .setThumbnail((await robloxClient.getPlayerThumbnail(user.userId, '150x150', 'png', false, 'headshot'))[0].imageUrl)
        .setFooter({ text: `User ID: ${user.userId}` })
        .setTimestamp()
        .addFields([
            {
...
export const getRoleListEmbed = (roles: any[]): EmbedBuilder => {
    const embed = new EmbedBuilder()
...
export const getJoinRequestsEmbed = (joinRequests: any[]): EmbedBuilder => {
...
export const getSuccessfulGroupBanEmbed = (user: any) : EmbedBuilder => {
    const embed = new EmbedBuilder()
        .setAuthor({ name: 'Success', iconURL: checkIconUrl })
        .setColor(greenColor)
        .setDescription(`**${user.username}** has successfully been banned from the group.`);
    
    return embed;
}

export const getSuccessfulGroupUnbanEmbed = (user: any) : EmbedBuilder => {
    const embed = new EmbedBuilder()
        .setAuthor({ name: 'Success', iconURL: checkIconUrl })
        .setColor(greenColor)
        .setDescription(`**${user.username}** has successfully been unbanned from the group.`);
