...
export class CommandContext  {
    ...
    constructor(payload: BaseInteraction | CommandInteraction | Message, command: any, args?: Args) {
        ...
        if(payload instanceof BaseInteraction) {
            ...
            ((interaction as any).options.data as any[]).forEach(async (arg) => {
                this.args[arg.name] = ((interaction as any).options).get(arg.name).value;
            });
        } else {
            ...
            if(filledOutArgs.length < requiredArgs.length) {
                ...
                if(args.length > requiredArgs.length) {
                    ...
                    const filteredKeys = Object.keys(this.args).filter((key) => !this.command.args.find((arg) => arg.trigger === key).isLegacyFlag);
                    const lastKey = filteredKeys[filteredKeys.length - 1];
                    this.args[lastKey] = [ this.args[lastKey], ...extraArgs.map((arg) => arg.value)].join(' ');
                }
    ...
    async reply(payload: string | InteractionReplyOptions | MessageCreateOptions | InteractionReplyOptions) {
        ...
        if(this.subject instanceof CommandInteraction) {
            ...
            return await (this.subject.channel as any).send(payload as MessageCreateOptions);
        }
    }
